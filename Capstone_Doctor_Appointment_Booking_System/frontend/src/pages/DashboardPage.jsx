...
function ProfilePanel({ user, onRefresh }) {
  ...
  const deactivateAccount = async () => {
    if (window.confirm("Are you sure you want to deactivate your doctor account? You will be logged out and reactivating will require admin approval.")) {
      try {
  ...
  return (
    <section className="panel-section profile-panel">
      ...
      {user?.role === 'DOCTOR' && (
        <div style={{ marginTop: '2rem', padding: '1.2rem', border: '1px solid var(--red)', borderRadius: '8px', background: '#fffefe' }}>
          <h3 style={{ color: 'var(--red)', margin: 0 }}>Danger Zone</h3>
          <p style={{ margin: '0.4rem 0 0.8rem', fontSize: '0.85rem', color: 'var(--muted)' }}>
            Deactivating your profile hides you from patient searches and prevents you from logging in. You can request activation later, which requires admin review.
          </p>
          <button type="button" style={{ background: 'var(--red)', color: 'white', border: 0, padding: '0.6rem 1rem', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }} onClick={deactivateAccount}>
        ...
        <section className="panel-section">
          ...
          {pendingPayment ? (
            <article className={`payment-panel ${paymentDone ? 'paid' : ''}`}>
              ...
              {!paymentDone && (
                showPaymentForm ? (
                  <form onSubmit={(e) => { e.preventDefault(); completePayment(); }} style={{ marginTop: '1rem', display: 'grid', gap: '0.8rem', maxWidth: '320px' }}>
                    <label style={{ display: 'grid', gap: '0.3rem' }}>Card Number
                      <input 
                        type="text" 
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.8rem' }}>
                      <label style={{ display: 'grid', gap: '0.3rem' }}>Expiry Date
                        <input 
                          type="text" 
                          placeholder="MM/YY" 
                          required 
                          ...
                          title="Expiry date as MM/YY" 
                        />
                      </label>
                      <label style={{ display: 'grid', gap: '0.3rem' }}>CVV
                        <input 
                          type="password" 
                          placeholder="123" 
                          required 
                    ...
                    <button type="submit" style={{ marginTop: '0.5rem' }}>Pay Now</button>
                    <button type="button" className="page-back" style={{ marginTop: '0.2rem' }} onClick={() => setShowPaymentForm(false)}>Cancel</button>
                  </form>
                ) : (
                  <button onClick={() => setShowPaymentForm(true)}>Complete payment</button>
                )
              )}
            </article>
